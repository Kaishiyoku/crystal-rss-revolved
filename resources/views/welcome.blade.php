    ...
    <body class="antialiased bg-gray-100 dark:bg-gray-900 pb-4 dark:text-gray-100">
            ...
            <div class="md:flex md:justify-between">
                ...
                @if (Route::has('login'))
                    <div id="navigation" class="md:fixed md:top-0 md:right-0 px-4 md:px-6 py-4 rounded-bl-2xl transition ease-out duration-500">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="text-primary-200 hover:text-white uppercase pl-4 transition-all duration-200 tracking-wider">{{ __('Dashboard') }}</a>
                        @else
                            <a href="{{ route('login') }}" class="text-primary-200 hover:text-white uppercase pl-4 transition-all duration-200 tracking-wider">{{ __('Log in') }}</a>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 pt-48 sm:pt-56 md:pt-64 lg:pt-80">
            <div class="flex flex-col justify-center items-center pt-8 sm:pt-0">
                ...
                <span class="text-6xl md:text-8xl text-center pt-4 text-transparent bg-clip-text bg-gradient-to-r from-primary-500 to-secondary-500">
            ...
            <div class="max-w-xl mx-auto mt-16 px-4 sm:px-0 py-4">
                ...
                <div class="mt-16 space-x-4 text-xl">
                    @if (config('app.contact_email'))
                        <x-link href="mailto:{{ config('app.contact_email') }}" class="text-primary-500 dark:text-secondary-500">
                            {{ __('Contact') }}
                        </x-link>
                    @endif

                    @if (config('app.github_url'))
                        <x-link href="{{ config('app.github_url') }}" class="text-primary-500 dark:text-secondary-500">
                            {{ __('GitHub') }}
                        </x-link>
                    @endif

                    <x-link href="{{ url('/docs') }}" class="text-primary-500 dark:text-secondary-500">
        ...
        <button type="button" class="block mx-auto mt-12 text-primary-500 dark:text-secondary-500 opacity-50 hover:opacity-75 focus:opacity-90 transition" data-scroll="#screenshots">
