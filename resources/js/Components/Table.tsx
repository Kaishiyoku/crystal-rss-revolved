...
const Table = ({children}: {children: ReactNode;}) => {
    return (
        <table className="min-w-full border border-gray-300 dark:border-gray-500 border-separate border-spacing-0 rounded-lg contain-paint">
            {children}
        </table>
    );
};
...
const HeadingCell = ({children, hideOnMobile = false, className = ''}: {children: ReactNode; hideOnMobile?: boolean; className?: string;}) => {
    return (
        <th
            className={clsx(
                'sticky top-0 z-10 border-b border-gray-300 dark:border-gray-500 bg-gray-100/50 dark:bg-gray-800/50 py-3.5 pl-4 pr-3 text-left font-semibold text-gray-900 dark:text-gray-100 backdrop-blur backdrop-filter sm:pl-6 lg:pl-6',
                className,
                {
